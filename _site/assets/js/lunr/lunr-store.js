var store = [{
        ...
        "title": "[aws]스트래스테스트",
        "excerpt":"**인스턴스 접속 : ssh -i 키위치/키이름.pem ubuntu@ip** **컴퓨터의 CPU 점유율 보기 : top ** ssh -i Desktop/dev/key/aws_password.pem ubuntu@54.180.2.171 **sudo apt-get update : 컴퓨터에 프로그램 목록을 최신버전으로 업그레이드** **ab 프로그램 : 아파치에서 만든 부화발생기 (인스턴스 접속 후 ab 타이핑)** **requests : 어떤 웹서버에 몇번의 접속을 시도하겠냐** **concurrency : 동시에 몇번의 접속을...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/aws-stress/",
        "teaser": null
      },{
        ...
        "title": "[aws]인스턴스 에서 웹서버 사용",
        "excerpt":"1. 홈페이지로 로그인해 연결을 눌러 키를 터미널에 복사 붙여넣기 를 해서 접속한다. 2. 아마존 EC2 인스턴스에 퍼블릭 DNS(IPv4)에 있는 주소를 복사해 주소창에 붙여넣기를 한다. 3. 홈페이지에 우분투 라고 나오면 제대로 접속한게 맞는것이다. ","categories": [],
        "tags": [],
        "url": "http://localhost:4000/aws-instance/",
        "teaser": null
      },{
        ...
        "excerpt":"# 터미널로 접속 server 켜기 1. ssh key 를 받아 안전한 폴더에 저장 2. aws 홈페이지 EC2 에 나의 인스턴스를 우클릭 후 연결을 누르면 접속 방법이 나온다 3. 인스터스를 킨다. 4. ssh 키를 안전한 폴더(그냥 디렉토리를 만들면 된다.)로 이동 시킨다. 5. ssh 키가 있는 디렉토리에서 chmod 400 aws_password.pem 실행 (만약,...","categories": [],
        "tags": [],
        ...
        "excerpt":"> Amazon machine image : 컴퓨터가 갖고 있는 상태를 그대로 얼려서 나중에 똑같이 복원하는 데이터 (그컴퓨터에 운영체제 ,소프트웨어 설정 ,현재 사용중인 프로그램 등등) 백업 기능이라 생각하면 된다 Wordpress : 가상머신의 형태로 제공된 솔루션을 사고 팔 수 있는 공간이라고 보면 된다.(다른사람이 만든 인스턴스를 사용하는 것) # wordpress 설치방법 1. aws markplace...","categories": [],
        "tags": [],
        ...
        "title": "[AWS] Elastic Ips & Scale Up",
        "excerpt":"# **Elastic Ips(탄력적 IP)** instances 를 다시 껏다가 키면 Public IP 가 달라진다. why? ip의 고갈현상이랑 밀접한 관계가 있다. 00.000.00.000 부터 255.255.255.255 까지 IP가 존재할 수 있다 약 (40억개) 이세상에 40억개만 IP만 가질 수 있다. (40억대만 인터넷을 사용할 수 있다) 그래서 내가 쓰는 IP 를 컴퓨터를 킬때마다 배분해 준다. 끄면...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/aws-elastic-ips/",
        "teaser": null
      },{
        ...
        "title": "[AWS]터미널로 서버접속, ELB적용(ab 테스터)",
        "excerpt":"1. 두개의 인스턴스를 생성해준다. 2. $ ssh -i 터미널경로/aws_passward.pem ubuntu@public ip주소 3. $ sudo apt-get update 타이핑(ab 벤치마킹 테스터기, 업데이트 먼저 해줘야함)패키지 리스트 를 업데이트를 해준다. 4. $ sudo apt-get install apache2-utils 타이핑(ab벤치마킹 설치), 웹서버 설치는 $ sudo apt-get install apache2 ELB적용 1. 웹서버를 이미지를 만들어준다.(AMI) 2. load balances 를...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/aws-elbab/",
        "teaser": null
      },{
        ...
        "title": "[AWS]Auto Scaling (오토스케일링)",
        "excerpt":"컴퓨터가 필요하면 자동으로 생성하고 필요없으면 자동으로 삭제해준다.(과금이 발생하지 않게 해주는 기능) AUTO SCALING 목록에 보면 시작구성과, Auto Scaling group 이 있다. 먼저 Auto Scaling group에 먼저 생성을 해준다. 그러면 1. 오토스케일의 이미지를 선택한다 2. 이미지화 시켯던 내AMI를 선택한다. 3. 인스턴스 성능 선택한다. 4. name 정하고 스토리지 정하고 5. 시큐리티 그룹을...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/awsauto-scaling/",
        "teaser": null
      },{
        "title": "[nodejs] web server, 변수, Template literals,URL",
        "excerpt":"web browser 가 web server 한테 정보를 요청하면 web server 그정보를 찾아서 web browser에게 응답해준다. # **DataType** String ```javascript console.log('asdjlkasdvnlkfnvljkfndjlvndfljvnjldfvdf.length) // 갯수를 확인 할 수 있다 ``` # **Variable(변수)** ```javascript a = 1; //대입연산자 1은 상수 console.log(a); a = 2; //a는 변수 (변할 수 있다), 2는 상수 (항상 상, 언제나...","categories": [],
        "tags": [],
        ...
        "title": "[AWS] Scale Out",
        "excerpt":"# 여러대의 컴퓨터가 협력해서 어떤 동일한 목표를 달성하는 컴퓨터들의 사회를 만든다 라고 할 수 있다. **처음에 Scale up 을 하다보면 단일 컴퓨팅으로써 한계에 도달하게 되는데 그 한계에 도달했을때 한계를 뛰어넘을 수 있는건 Scale out** 데이터를 처리할떄 보통 web server - middle ware - database 순이다. web server 대표적 으로 nginx,...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/aws-scale-out/",
        "teaser": null
      },{
        "title": "[얄코]Cookie, Session, Cache",
        "excerpt":"# 쿠키 : 브라우저에 저장되는 내용들 , 내컴퓨터에 있으닌깐 내가 갖고 있는 정보들 하지만 쿠키는 한계가 있다 내가임의로 고치거나 고칠 수 있고 남이 쉽게 볼 수 도있다. ( 로그인자동완성, 오늘 하루 안보기, 쇼핑을 하는데 로그인을 하지않고 물건을 장바구니에 담는 등 지워지거나, 조작되거나, 가로채거나 보여져도 큰 지장이없는 정보들을 브라우져에 저장하는데 사용됨)...","categories": [],
        "tags": [],
        ...
        "title": "[얄코]객체지향 프로그래밍",
        "excerpt":"**객체지향**이랑 어떤 프로그래밍 방식, 방법론이다. ( 객체지향 언어는 복잡한 코드를 간결하게? 해주는 것이라 생각하면 됨.) 데이터와 기능이 클래스로 '캡슐화' 된 컴퓨터 자원의 묶음을 '객체'라고 한다. 밀가루 반죽과 팥이 클래스란 붕어빵이 틀에 찍혀서 객체라는 붕어빵이 나온다. **'은닉성'** : 우리가 TV 속을 뜯어보지 않아도 리모콘으로 필요한 조작을 다 할 수 있는 것처럼...","categories": [],
        "tags": [],
        ...
        "title": "[node]정적파일 서비스",
        "excerpt":"images 디렉토리 만들어주고  ```javascript app.use(express.static('public'));//public 디렉토리 안에서 static 파일을 찾겠다. ```  그다음 본문 로직에 ```javascript  ``` ","categories": [],
        "tags": [],
        "url": "http://localhost:4000/nodejs-silence/",
        "teaser": null
      },{
        ...
        "title": "[nodejs] Express 미들웨어",
        "excerpt":"Express 의 중요 기능 2가지 1. 라우터 2. 미들웨어 소프트웨어를 만들때 처음부터 끝까지 다만드는 경우는 없다. 다른사람이 만든 소프트웨어를 부품으로 해서 만든다. 이것을 미들웨어라고 생각하면 된다. ```javascript npm install body-parser //로설치 ``` body 는 웹브라우저에서 요청한 정보 본체를 뜻한다. parser 는 분석 ```javascript var bodyParser = require('body-parser'); //맨위에 선언 var...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/nodejsexpress/",
        "teaser": null
      },{
        "title": "[nodejs] Escaping (탈출)",
        "excerpt":"저장된 정보가 밖에 나올때 필터링해서 안보이게? 해주는 것 (  ```javascript var sanitizeHtml = require('sanitize-html'); ``` ","categories": [],
        "tags": [],
        ...
        "title": "[얄코]포인터",
        "excerpt":"철수와 영희가 있는데 철수가 팀프로젝트에서 필요한 교과서에 페이지를 알고있다 이거를 영희에게 알려줄려하는데 종이로 베껴서 보여주면 종이낭비닌깐 교과서의 페이지만 알려준다 이것이 포인터 ``` char language = 'c'; int thisYear = 2019; double pi = 3.14; ``` 이러한 변수에 입력된 값들은 메모리 어딘가에 저장 되어 있다. 메모리는 어떠한 값들이 나란히 정렬되어 있지...","categories": [],
        "tags": [],
        ...
        "excerpt":"```java app.get('/create',function(request,response){ //get방식일떄  app.post('/create_process',function(request,response){ //post방식 ```  쿼리 스트링이 아닌 방식으로 바꿀려면  ```java update 에서 update ```   삭제기능 ```java  //action 에 delete 앞에 /를 주게되면 최상위 path 바로밑으로 감 ```   express 는 리다이렉션 기능을 편리하게 제공하고 있다.  ```java response.redirect //를 사용하면 된다 ``` ","categories": [],
        "tags": [],
        ...
        "title": "[nodejs] express 홈페이지 구현",
        "excerpt":"기존 라우팅 방식은  ```java var app = http.createServer(function(request,response){} ```  많은 부분에서 공유하고 사용했다. 어떻게 공유 되는 부분인지 확인할려면 위로올라서 확인해야하는데.  express 방식의 라우팅 방식으로 하면  request,response의 개발할려는 로직에  필요한 것만 들어있어서 간편하게 사용할 수 있다.  path 방식 url 파라미터 처리방법 ```java app.get('/page/:pageId/:chapterId', function(request, response){   return response.send(request.params);   //{\"pageId\":\"HTML\",\"chapterId\":\"css\"} })  ``` ","categories": [],
        "tags": [],
        "url": "http://localhost:4000/nodejs-express/",
        "teaser": null
      },{
        "title": "[Nodejs]파일생성, 리다이렉션, 파일삭제",
        "excerpt":"**파일생성과 리다이렉션** >리다이렉션 : 출력의 방향을 바꾼다. ```java response.writeHead(302, {Location : 'http://whateverhostthiswillbe:8675/' +newRoom}); response.end(); ``` writeHead(200) 은 성공시켰다. writeHead(302) 페이지를 다른데로 리다이렉션 시킨다. ex) ```java fs.writeFile(`data/${title}`, description, 'utf8', function(err){ response.writeHead(302, {Location: `/?id=${title}`}); response.end(); }) ``` > fs.unlink(path, callback) => 파일삭제 ex) ```java fs.unlink(`data/${id}`, function(error){ response.writeHead(302, {Location: `/`}); response.end(); }) ```...","categories": [],
        "tags": [],
        ...
        "excerpt":"파일을 저장하는 서비스 라고 생각하면 된다.  1. s3 클릭한뒤 버킷을 생성한다. 2. 버킷생성 뒤 파일을 업로드 해본다.  ex) 를 들어 웹사이트를 운영한다.그러면 그 사이트에서 사용자가 파일을 업로드하면 미들웨어(루비, 파이썬, 자바 등등)가 s3 의 api 를 통해 s3로 전송한다. ","categories": [],
        "tags": [],
        ...
        "excerpt":"```javacript app.use('/topic',topicRouter) //topic 라우터를 타면 topicRouter 라는 미들웨어를 적용시키겠다. router.get('/create',function(request,response){ var title = 'WEB - create'; var list = template.list(request.list);//미들웨어 var html = template.HTML(title, list, ` `, ''); response.send(html); } ``` routers 에 위치한 topic.js 를 만든뒤 path 하는 메소드 들을 전부다 이주 시켜주었다. ```javascript var express = require('express'); var...","categories": [],
        "tags": [],
        ...
        "title": "[nodejs]보안",
        "excerpt":"1. express 를 항상 최신버전으로 유지. 2. https 를 써라 why? 웹브라우저와 웹서버가 통신할때 암호화를 하기떄문에. 3. 단골손님같은 보안이슈를 자동으로 해결해준다. ``` npm install -save helmet ``` 4. 쿠키를 올바르게 사용해라(누가들어왓는지 확인하는용도?) 5. 웹애플리케이션이 쓰고있는 모듈들을 잘 관리해라 ``` npm i nsp -g ``` ","categories": [],
        "tags": [],
        ...
        "title": "[nodejs]쿠키",
        "excerpt":"웹브라우저가 서로 주고받는 데이터 http 프로토콜에 포함되어있는 기술 쿠키생성하는방법 ```javascript var http = require('http'); http.createServer(function(request, response){ // response.writeHead(200, { //성공했다는 가정하에 2번쨰는 객체를 넣어준다. // 'Set-Cookie':['yummy_cookie=choco', 'tasty_cookie=strawberry'] //두개의 쿠키가 생성됨. // }) response.end('Cookie!!!'); }).listen(3000); ``` 쿠키를 서버에 요청하는 법 먼저 npm 을 다운로드한다 ``` npm install -s cookie ``` 그다음...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/nodejs-cookie/",
        "teaser": null
      },{
        ...
        "title": "[nodejs] 쿠키 실습",
        "excerpt":"로그인 상태체크 ```javascript function authIsOwner(request,response){ //쿠키 체크 var isOwner = false;//기본으로 false if(request.headers.cookie){//쿠키 값 유무 체크 cookies = cookie.parse(request.headers.cookie); } if(cookies.email === 'egoing777@gmail.com' && cookies.password === '11111'){ isOwner = true; } console.log(cookies) return isOwner; } ㅎ var app = http.createServer(function(request,response){ var isOwner = authIsOwner(request,response); } ``` 로그인 상태를 UI에 반영...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/nodejs-cookie2/",
        "teaser": null
      },{
        ...
        "title": "[nodejs]express(Hello World!)",
        "excerpt":"nodejs 에서 가장많이 사용하는 프레임웤 npm install express --save 으로 설치 ```java var express = require('express') //express 모듈을 load 해온다 const 는 상수 바뀌지않음. var app = express() //express 호출 (함수) app.get('/' , (req, res) => res.send('/'))//첫번째 인자는 경로, 두번째는 접속했을때 호출될 함수가 표현되어있음.(라우터라 생각하면됨) //(라우팅라 생각하면됨(라우팅은 경로 즉 길을가고있는데...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/nodejs-expresshello-world/",
        "teaser": null
      },{
        "title": "[nodejs] 객체화,모듈사용법,경로보안,출력정보 보안",
        "excerpt":"# 코드를 간결하게 객체화 해서 사용가능 다른말로 리팩토링 이라고도 함 ```javascript var template = { //template 라는 객체를 생성 HTML:function(title, list, body, control){ return ` WEB1 - ${title} WEB ${list} ${control} ${body} `; },list:function(filelist){ var list = ''; var i = 0; while(i ${filelist[i]}`; i = i + 1; }...","categories": [],
        "tags": [],
        ...
        "title": "[nodejs]에러처리",
        "excerpt":"존재하지 않는 페이지를 찾았을때 404 에러가 나타나는데. express 기준  ```javascript app.use(function(req, res, next){   res.status(404).send('Sorry cant find that!'); }) //추가해준다 ``` 페이지에 에러가 있을 경우(서버에러 500)  ```javascript  next(err); //다음 미들웨어를 호출하는 것 에러를 던진다 생각하면 된다.      app.use(function(err, req, res, next){ //첫번째는 next에 담긴 err가 넘어오고   res.status(500).send('Something broke!'); }) ``` ","categories": [],
        "tags": [],
        "url": "http://localhost:4000/nodejs-error/",
        "teaser": null
      },{
        ...
        "excerpt":"component 는 HTML 을 반환하는 함수다. 사용자정의 태그를 만들어주는 여러가지 기술이있는데 리액트 도 그중에 하나 사용자가 정의 해서 만드는걸 컨포넌트라 함  1. 가독성을 높힘 2. 재사용성이 좋음 3. 유지보수가 좋아짐   리액트는 항상 파일 위에다가 import React from 'react'; <- 써줘야함 아니면 JSX 인식못함 ","categories": [],
        "tags": [],
        ...
        "title": "[React]생활코딩 기초",
        "excerpt":"```javascript ReactDOM.render(         //사용자 정의에 의한 컨포넌트   ,   document.getElementById('root') ); ``` ","categories": [],
        "tags": [],
        ...
        "excerpt":"*JSX : 페이북에서 만든 컴퓨터 언어* 우리가 JSX 로 코드를 작성하면 create-react-app 이 알아서 javascript 의 언어로 컨버팅 해준다. # 컨포넌트 생성방법 ```javascript class Subject extends Component { render(){ //반드시 있어야함 class 안에 function 생략 return( //하나의 최상위 태그만 써야한다. WEB world wide web! ); } } class App extends...","categories": [],
        "tags": [],
        ...
        "title": "[React] props (프로퍼티)",
        "excerpt":"상위(부모) 컴포넌트에서 하위(자식) 컴포넌트에게 데이터를 전달 할때 사용   ```javascript class Subject extends Component {   render(){ //반드시 있어야함 class 안에 function 생략     return( //하나의 최상위 태그만 써야한다.                       {this.props.title}              {this.props.sub}              );   } }   class App extends Component {   render() {     return (                                                   );   } } ``` ","categories": [],
        "tags": [],
        "url": "http://localhost:4000/react-props/",
        "teaser": null
      },{
        "title": "[React] state 사용법 (key)",
        "excerpt":"```javascript //App.js class App extends Component { constructor(props){ //컨포넌트 초기화 super(props); this.state = { //스테이트 값 초기화 subject:{title:'WEB', sub:'world wide Web'} } } render() { return ( ); } } ``` ```javascript //TOC.js import React,{Component} from 'react'; class TOC extends Component { render(){ var lists = []; var data =...","categories": [],
        "tags": [],
        ...
        "excerpt":"```javascript import React,{Component} from 'react'; // react 라는 라이브러리 에서 Component 라는 클래스를 갖고온다 ``` src 밑에 component 라는 디렉토리 생성 후 TOC.js 를 만든다. ```javascript import React,{Component} from 'react'; class TOC extends Component { render(){ return( HTML CSS Javascript ); } } export default TOC; //TOC라는 파일안에 여러가지 변수나...","categories": [],
        "tags": [],
        ...
        "excerpt":"```javascript //App.js          ``` onChangePage 라는 이벤트를 생성   ```javascript //Subject.js  import React,{Component} from 'react'; class Subject extends Component {   render(){ //반드시 있어야함 class 안에 function 생략     return( //하나의 최상위 태그만 써야한다.                       {this.props.title}              {this.props.sub}              );   } }  ``` this.props.onChangePage() 호출하면 된다. ","categories": [],
        "tags": [],
        ...
        "title": "[React]CRUD (배열)",
        "excerpt":"//createContent.js ```javascript import React,{Component} from 'react'; class CreateContent extends Component{ render(){ return( Create ); } } export default CreateContent; ``` //ReadContent.js ```javascript import React,{Component} from 'react'; class ReadContent extends Component{ render(){ return( {this.props.title} {this.props.desc} ); } } export default ReadContent; ``` 배열에 값을 넣을떄 ``` var arr = [1,2]; arr.push(3);...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/react-CRUD/",
        "teaser": null
      },{
        "title": "[React]JSX, Component",
        "excerpt":"JSX : HTML + javascript (javascript 안의 HTML 이라 생각하면 됨) ``` ReactDOM.render( , document.getElementById('root') ); ``` 에러가 발생한다 wh? react aaplication이 하나의 component 만을 rendering 해야 하기 때문이다. ``` function Potato (){ return I like Potato } function App() { return ( Hello ); } ``` 정상적으로 잘 작동한다....","categories": [],
        "tags": [],
        ...
        "excerpt":"```javascript const friends = [\"dal\",\"mark\",\"lynn\",\"japan guy\"] friend.map(function(current) { console.log(current); return 0 }) result : dal mark lynn japan guy ``` return 값에 item 을 각각 매칭 시켜준다 ```javascript const friends = [\"dal\",\"mark\",\"lynn\",\"japan guy\"] friend.map(function(current) { console.log(current); return current + \"10\" }) result : dal10 mark10 lynn10 japan guy10 ``` ```javascript...","categories": [],
        "tags": [],
        ...
        "title": "[React Nomad]State",
        "excerpt":"state 는 보통 우리가 동적 데이터와 함께 작업할때 만들어진다. 변하는 데이터, 존재하지 않는 데이터 ```javascript class App export React.Component //컨포넌트를 만들때 마다 모든 것을 다 구현하고 싶지 않아서. state ={ } //state는 object이고 component의 data를 넣을 공간이 있고 이 데이터는 변한다. render(){ //기본적으로 render 메소드를 갖고있음react 는 자동적으로 나의 class...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/react-nomad-State/",
        "teaser": null
      },{
        ...
        "title": "[React Nomad]State 2",
        "excerpt":"``` state = { count: 0 } add = () => { this.state.count = 1; }; minus = () => { this.state.count = -1; }; //동작하지 않음. react 는 render function 을 refresh 하지 않는다. ``` ``` state = { count: 0 } add = () => { this.setState({count :...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/react-nomadstate-2/",
        "teaser": null
      },{
        ...
        "title": "[React Nomad] Component Life Cycle",
        "excerpt":"# **Mountig** constructor() : javascript 에서 class를 만들 때 호출되는것. ``` constructor(props){ super(props) console.log(\"hello\"); } componentDidMount(){ console.log(\"component rendered\") } render(){ console.log(\"I m rendering\") } //constructor() 가 가장먼저 실행 그다음 render() 그다음 componentDidMount ``` # **Update** 내가 Add 혹은 Minus를 클릭해서 state를 변경할때 그때가 update ``` componentDidUpdate(){ console.log('I just update') }...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/react-nomad-coder-component-life-cycle/",
        "teaser": null
      },{
        ...
        "title": "[React Nomad]Style the movie",
        "excerpt":"App.js ```javascript import \"./App.css\";     render() {     const { isLoading, movies } = this.state;     return (                {isLoading ? (                        Loading...                    ) : (                        {movies.map(movie => (                            ))}                      )}            );   } ```  movie.js ```javascript import \"./Movie.css\";  function Movie({year, title, summary, poster}){     return                                {title}              {year}              {summary}                 } ``` ","categories": [],
        "tags": [],
        "url": "http://localhost:4000/react-nomadstyle-the-movie/",
        "teaser": null
      },{
        "title": "[React Nomad]Map &크롤링 스크래핑 차이",
        "excerpt":"아래는 map()함수를 이용하여 numbers 배열의 값을 두배로 만든 후 map()에서 반환하는 새 배열을 doubled 변수에 할당하고 로그를 확인하는 코드입니다. ```javascript const numbers = [1, 2, 3, 4, 5]; const doubled = numbers.map((number) => number * 2); console.log(doubled); ``` 이 코드는 콘솔에 [2, 4, 6, 8, 10]을 출력합니다. React에서 배열을 엘리먼트...","categories": [],
        "tags": [],
        ...
        "excerpt":"```javascript import {HashRouter, Route} from \"react-router-dom\"; //react-router-dom은 다른 종류의 라우터가 있다 그중에 하나가 HashRouter,Route ``` ```javascript function App(){ //함수형 컨포넌트 생성 return } ``` 2개의 컨포넌트가 같이 렌더링이 된다. why? 리액트는 쉽게 말해 자신이 찾는 문자가 속해 했으면 그것도 렌더링을 해버리기 때문? 그래서 path=\"/\" 을 먼저 렌더링 한뒤, path=\"/about\" 도...","categories": [],
        "tags": [],
        ...
        "excerpt":"```Javascript //App.js function App(){ //함수형 컨포넌트 생성 return } ``` ```javascript function Navigation(){ return HOme About } ``` Link 는 반드시 route 안에 있어야한다. 밖에 있으면 동작이 안됨. ``` function App(){ //함수형 컨포넌트 생성 return } ``` HashRouter 대신 BrowserRouter 을 바꾸면 http 사이트에 /#/ 가없어짐 하지만 git에 설정하기가 짜증남....","categories": [],
        "tags": [],
        ...
        "excerpt":"함수형 컴포넌트는 ```javascript function App() { const test = \"\"; return {test} } ``` 클래스형 컴포넌트의 형태는 ```javascript class App extends Component { render(){ const test = \"\"; return {test}; } } ``` 함수형 컴포넌트의 장점 - 클래스형 컴포넌트에 비해 선언하기가 편하다. - 메모리 자원을 클래스형 컴포넌트에 비해서 덜 차지한다....","categories": [],
        "tags": [],
        ...
        "title": "[얄코]Scope (let, const, var)",
        "excerpt":"각 상수나 변수 등의 요소들이 허용된 영역, 범위를 뜻 함. 예를들어 전역번수로 선언된 함수를 스코프 안쪽에 if(), for() 안 쪽에 선언을 하면 안쪽에 있는 변수로 접근하게 된다. ```javascript const mouse_1 = '서울쥐'; mouse_1 '귀촌쥐'; // 값 을 바꿀수없는 상수 const 에서 바로 선언을 해주고 값도 바로 넣어줘야함 / ES6 ECMA...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/ffa/",
        "teaser": null
      },{
        ...
        "title": "[Nodejs]용어2",
        "excerpt":"exports : 모듈을 만들어 주는 대표적인 방식 ```javascript //정의 exports.함수이름 = 함수정의; //쓰는법 var modeule = require=('module'); module.함수이름(); ``` 자바스크립트는 함수라는 것을 명시적으로 구별하기 위해 function 키워드를 함수 이름 앞에 붙여줌 ```javascript //같은의미이다. fucntion add(a,b){ return a+b; } var add = function(a,b){ return a+b; } ``` 배열요소 삭제 ```javascript var...","categories": [],
        "tags": [],
        ...
        "excerpt":"**주소 문자열을 URL 객체로 변환** ```javascript var url = require('url'); //모듈호출 //parse() 와 format() 메소드를 호출 할 수 있음 parse() : https:// 로 시작하는 주소 문자열을 URL 객체로 만들어줌 (요청 파라미터 문자열을 파싱하여 요청 파라미터 객체로 만들어 줌) format() : 원래의 주소 문자열로 돌려놓음(요청 파라미터 객체를 문자열로 변환) ``` **이벤트**...","categories": [],
        "tags": [],
        ...
        "excerpt":"노드의 기본 모듈인 http 를 사용하면 웹 서버 기능을 담당하는 서버 객체를 만들 수 있습니다. ```javascript var http = require('http'); var server = http.createServer(); //웹서버 객체 만들기 var port = 3000; server.listen(port, function(){ console.log('웹서버가 시작되었습니다',port); }); //list(port,[hostname],[backlog],[callback]) 서버를 실행하여 대기시킵니다 //close([callback]) 서버를 종료합니다 ``` 웹 서버는 일반저그올 웹 브라우저라고 하는...","categories": [],
        "tags": [],
        ...
        "title": "[Nodejs]객체",
        "excerpt":"# Object 와 Array ```java var members = ['egoging','k8805','hoya']; console.log(members[1]); var i = 0; while (i',name, 'value =>',roles[name]); } ``` # objeck oriented Programming (OOP) ```java //자바스크립트 에게 함수는 값이기도하고 , 그렇기에 배열과 객체에 담을 수 있다. var f = function() { console.log(1+2); console.log(1+2); } var a = [f]; a[0]();...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/nodejs-object/",
        "teaser": null
      },{
        ...
        "excerpt":"# 쿠키 와 세션 **쿠키 : 클라이언트 웹 브라우저에 저장되는 정보** **세션 : 웹 서버에 저장되는 정보** 익스프레스 에서는 cookie-parser 미들웨어를 사용하면 쿠키를 설정하거나 확인 할 수 있다. 세션은 쿠키와 달리 서버 쪽에 저장 대표적인 예 : 로그인 ```javascript var cookieParser = require('cookie-parser') // 쿠키 모듈 var expressSession = require('express-session')...","categories": [],
        "tags": [],
        ...
        "title": "[JAVA]기본",
        "excerpt":"# 자바 동작원리 Java Source code(.java) ----Compile----> Java Application(.class) -----Run------> Java Virtual Machine ----------Run--------> computer 자바파일을 저장버튼을 눌렀을 때 .class 란 확장자를 가진 파일이 만들어진다. 그것이 Java Application 그다음 우리가 실행 버튼(run) 을 누르게되면 Java Virtual Machine이 확장자가 class 를 가진 파일을 읽어서 컴퓨터를 동작시키게 된다. # Operator ```java System.out.println(Math.PI);...","categories": [],
        "tags": [],
        ...
        "title": "[JAVA] 기본 코딩 & 디버깅",
        "excerpt":"```java public class OkJavaGoInHome { public static void main(String[] args) { String id = \"JAVA APT 507\"; // Elevator call Elevator myElevator = new Elevator(id); myElevator.callForUp(1); // Security off Security mySecurity = new Security(id); mySecurity.off(); // Light on Lighting hallLamp = new Lighting(id+\" / Hall Lamp\"); hallLamp.on(); Lighting floorLamp =...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/java-coding&debugging/",
        "teaser": null
      },{
        ...
        "title": "[JAVA] 제어문",
        "excerpt":"# **제어문(조건문, 반복문)** 1-1 .boolean data type : 'boolean' 에는 true, false 가 있음 ```java System.out.println(foo.contains(\"world\")); //해당문자열을 포함 하고 있는지 확인시켜줌 (true / false) 로 return 해줌 ``` # 1-2 . Comparison Operator(비교연산자) ```java System.out.println(1>1); // false System.out.println(1 == 1); //true System.out.println(1 = 1); // true ``` # 1-3 conditional...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/java-Control_Statement/",
        "teaser": null
      },{
        ...
        "title": "[JAVA]access level modifiers & OOP",
        "excerpt":"# access level modifiers : 외부에서 접근 할 수 있는 레벨을 정해 주는 것. public : 어느 클래스에서도 접근이 가능 할 수 있다. protected : default : private : 같은 클래스 안 에서만 사용할 수 있다. but public 을 사용하면 된다.(내부적인 메소드) static - class method ( 클래스 소속이다.) no...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/javaaccess-level-modifiers-oop/",
        "teaser": null
      },{
        ...
        "title": "[JAVA]인스턴스 & static & 생성자",
        "excerpt":"# 인스턴스 ```java class Print { .... } public static void main(String[] args){ Print p1 = new Print(); // print 클래스를 복제한다. (print 라는 데이터 타입(클래스) 이다) .... } ``` # static :정적이다. ```java class Foo{ public static String classVar = \"I class var\"; public String instanceVar = \"I instance...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/java-static/",
        "teaser": null
      },{
        ...
        "title": "[JAVA]상속 & 차장님 기본개념",
        "excerpt":"# 상속 상속(Interitance): 부모 클래스의 메소드 및 필드를 가져옴으로써 코드 중복을 줄일 수 있는 장점 오버라이딩 : 부모 클래스에 있었던 메소드를 자식 클래스에서 재정의(덮어쓰기)하면서 코드 변경 가능 오버로딩 : 부모클래스가 갖고 있는 메소드의 형태를 자식클래스에서 조금 바꿔 \"이름을 같게\" 사용할 수 있다. (메소드의 매개변수 타입, 개수변화 등등) 원래 메소드란 이름이...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/java/",
        "teaser": null
      },{
        ...
        "title": "[nodejs] 파일생성, 리다이렉션, 파일삭제",
        "excerpt":"**파일생성과 리다이렉션** >리다이렉션 : 출력의 방향을 바꾼다. ``` response.writeHead(302, {Location : 'http://whateverhostthiswillbe:8675/' +newRoom}); response.end(); ``` writeHead(200) 은 성공시켰다. writeHead(302) 페이지를 다른데로 리다이렉션 시킨다. ex) ``` fs.writeFile(`data/${title}`, description, 'utf8', function(err){ response.writeHead(302, {Location: `/?id=${title}`}); response.end(); }) ``` > fs.unlink(path, callback) => 파일삭제 ex) ``` fs.unlink(`data/${id}`, function(error){ response.writeHead(302, {Location: `/`}); response.end(); }) ```...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/nodejs/",
        "teaser": null
      },{
        ...
        "title": "[Spring] 개념",
        "excerpt":"### Spring 이란? - 자바 엔터프라이즈 개발을 편리하게 해주는 **오픈소스 경량급 애플리케이션 프레임워크**이다. ### Spring의 특징 - **경량 컨테이너**로서 자바의 객체를 Spring이 직접 관리함 - 각각의 객체 생성, 소멸과 같은 라이프 사이클을 스프링이 대신 관리해주며, 스프링으로부터 객체를 얻어올 수 있다. - **제어 역행**(**IOC** : Inversion of Control) - 애플리케이션 간의...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/spring/",
        "teaser": null
      },{
        "title": "[JAVA]프로그래머스(값 뒤집기),타입",
        "excerpt":"```java import java.util.Arrays; class SolutionCoding { public static int[] solution(long n) { String input = \"\" + n; int size = input.length(); int[] answer = new int[size]; for (int i = size - 1; i >= 0; i--) { answer[i] = input.charAt(size - i - 1) - '0'; } return...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/java/",
        "teaser": null
      },{
        ...
        "title": "[JAVA]ArrayList",
        "excerpt":"# ArrayList란? ArrayList는 List 인터페이스를 상속받은 클래스로 크기가 가변적으로 변하는 선형리스트입니다. 일반적인 배열과 같은 순차리스트이며 인덱스로 내부의 객체를 관리한다는점등이 유사하지만 한번 생성되면 크기가 변하지 않는 배열과는 달리 ArrayList는 객체들이 추가되어 저장 용량(capacity)을 초과한다면 자동으로 부족한 크기만큼 저장 용량(capacity)이 늘어난다는 특징을 가지고 있습니다. ```java ArrayList list = new ArrayList();//타입 미설정 Object로...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/javaarraylist/",
        "teaser": null
      },{
        ...
        "title": "[JAVA]알고리즘chap01",
        "excerpt":"chap01 : 세 값의 대소 관계와 중앙값 구하기 ```java package chap01; import java.util.Scanner; public class Median { static int med3(int a, int b, int c) { if(a >= b) if(b >= c) return b; else if( a c) return a; else if(b > c) return c; else return b; }...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/javachap01/",
        "teaser": null
      },{
        "title": "[JAVA]알고리즘 chap02 - 1",
        "excerpt":"# 배열의 특성 확인 ```java package chap02; class IntArray { public static void main(String []args) { int[] a = new int[5]; //배열의 선언 a[1] = 37; a[2] = 51; a[4] = a[1] * 2; for(int i = 0; i max) max = a[i]; return max; } public static void main(String...","categories": [],
        "tags": [],
        ...
        "title": "[JAVA] 재귀함수",
        "excerpt":"```java package test;  public class Step1Test { \tstatic int sum = 0; \t \tstatic int sum(int param) { \t\tif(param != 0) { \t\t\tsum += param; \t\t\tparam--; \t\t\tsum(param); \t\t} \t\treturn sum; \t} \tpublic static void main(String[] args) { \t\tSystem.out.println(sum(Integer.parseInt(args[0]))); \t}  } ```  interface 는 private 를 못쓴다. ","categories": [],
        "tags": [],
        ...
        "title": "[JAVA]알고리즘 chap02 - 2 & String class",
        "excerpt":"String class 문자열을 나타내는 것이 java.lang 패키지에 소속된 String 클래스 이다. (기본형 int, double 형이 아니다) ```java String s = \"ABC\"; ``` 초기자의 \"ABC\"는 문자열 리터럴입니다. 문자열 리터럴은 단순히 문자가 늘어서 있는 것이 아니라 String형 인스턴스에 대한 참조입니다. String클래스는 문자열을 넣어두기 위한 문자 배열, 문자 수를 나타내는 필드 등을 갖고...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/java-chap02-2/",
        "teaser": null
      },{
        ...
        "title": "[JAVA]알고리즘 chap02 - 3 & Class",
        "excerpt":"# 2행 4열의 2차원 배열 ```java //2행 4열의 2차원 배열 package chap2; public class Int2DArray { public static void main(String[] args) { int[][] x = new int[2][4]; //2차원 배열 선언 x[0][1] = 37; x[0][3] = 54; x[1][2] = 65; for(int i = 0; i = 0.0 && dat[i].vision <= VMAX...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/java/",
        "teaser": null
      },{
        "title": "[JAVA]알고리즘 chap03 검색알고리즘",
        "excerpt":"# 선형 검색 ```java package chap03; import java.util.*; public class SeqSearch { //요솟수가 n인 배열a에서 key와 같은 요소를 선형 검색합니다. static int seqSearch(int[] a, int n, int key) { int i = 0; while(true) { if(i == n) return -1; //검색실패(-1을 반환) if(a[i] == key) return i; //검색 성공(인덱스를 반환)7...","categories": [],
        "tags": [],
        ...
        "title": "[JAVA]Study",
        "excerpt":"201112시험 – JAVA 15문 Spring 5문 1. 다음 각 변수의 생명 주기를 쓰시오. 지역 변수 : 메소드 시작 끝까지 인스턴스 변수 : 객체가 생성되고 가비지 컬렉션이 정리할 때 까지 정적 변수(클래스 변수) : 프로그램이 시작 되서 프로그램이 끝날떄 까지 자바에서 모든 예외를 처리할 수 있는 Class는 (①Exception)입니다. 추가로 가장 많이...","categories": [],
        "tags": [],
        ...
        "excerpt":"# 이진검색 ```java package chap03; import java.util.*; public class BinSearch { //요솟수가 n인 배열 a에서 key와 같은 요소를 이진 검색합니다. static int binSearch(int[] a, int n, int key) { int pl = 0; //검색 범위의 첫 인덱스 int pr = n - 1; //검색범위의 끝 인덱스 do { int pc...","categories": [],
        "tags": [],
        ...
        "excerpt":"# 자연정렬 ```java package chap03; import java.util.*; public class StringBinarySearch { public static void main(String []args) { Scanner stdIn = new Scanner(System.in); //자바에서 사용하는 키워드입니다. String[] x = { \"abstract\", \"assert\" }; System.out.println(\"원하는 키워드를 입력하세요:\"); String ky = stdIn.next(); int idx = Arrays.binarySearch(x, ky); //배열 x에서 값이 ky인 요소를 검색...","categories": [],
        "tags": [],
        ...
        "excerpt":"*스택이란 ?* 데이터를 일시적으로 저장하기 위해 사용하는 자료구조루, 데이터의 입력과 출력 순서는 후입선출(LIFO) 입니다. 스택에 넣는 작업을 push 꺼내는 작업을 pop *메서드종류* * indexOf (검색 메서드) : 스택 본체의 배열 에서 어떤 값의 데이터가 포함되어 있는지, 포함되어 있으면 배열의 어디에 들어 있는지를 조사하는 메서드 * clear (스택의 모든 요소를 삭제)...","categories": [],
        "tags": [],
        ...
        "title": "[JAVA]알고리즘 chap-04 queue",
        "excerpt":"*큐란?* 스택과 마찬가지로 데이터를 일시적으로 쌓아 두기 위한 자료구조입니다. 선입선출 구조입니다. # int형 큐(링 버퍼) ```java package chap04; public class IntQueue { private int max; //큐의 용량 private int front; //첫 번째 요소 커서 private int rear; //마지막 요소 커서 private int num; //현재 데이터 수 private int[] que; //큐...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/java-stack-queue/",
        "teaser": null
      },{
        "title": "[JAVA]Study2 (프로그래머스)-마라톤",
        "excerpt":"오름 차순 정렬으로 collention.sort() 이용하면 됨. hashset은 순서가 없기 떄문에 순서대로 꺼내기 위해 iterator라는 걸 사용. 데이터의 위치를 바탕으로 순차적으로 데이터를 쌓고 그걸 순차적으로 꺼내온다고 생각하면 된다. ```java package test20201126; import java.util.ArrayList; import java.util.Collections; import java.util.HashSet; import java.util.Iterator; public class Test { public static int[] solution(int[] numbers) { int[] answer...","categories": [],
        "tags": [],
        ...
        "excerpt":"# 재귀? 어떤 사건이 자기 자신을 포함하고 다시 자기 자신을 사용하여 정의될 때 재귀적이라고 합니다. 대표적으로 팩토리얼을 구하는 프로그램이 있다. ```java package chap05; import java.util.Scanner; public class Factorial { static int factorial(int n) { if(n>0) return n * factorial(n-1); else return 1; } public static void main(String[] args) { Scanner...","categories": [],
        "tags": [],
        ...
        "title": "[JAVA]chap05-재귀2 &chap06-정렬",
        "excerpt":"# 8퀸문제 ```java //8퀸 문제 package chap05; public class QueenB { static int[] pos = new int[8]; //각 열의 퀸의 위치 //각 열의 퀸의 위치를 출력합니다. static void print() { for(int i = 0; i i; j--) if(a[j - 1] > a[j]) swap(a,j - 1, j); } public static void...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/java-Recursion02/",
        "teaser": null
      },{
        ...
        "excerpt":"# 첫번째 ```java package test20201209; import java.util.ArrayList; import java.util.Collections; import java.util.List; class Solution2 { public int[] solution(int[] array, int[][] commands) { int[] answer = new int[commands.length]; List startList = new ArrayList(); List endList = new ArrayList(); List indexList = new ArrayList(); //for-each문 한정적 //배열, 리스트에만 사용이 가능해 //순서가 있는, 그리고...","categories": [],
        "tags": [],
        ...
        "title": "[JAVA]chap06-단순삽입정렬",
        "excerpt":"단순삽입정렬 ```java package chap06; import java.util.*; public class InsertionSort { static void insertionSort(int[] a, int n) { for (int i = 1; i 0 && a[j - 1] > tmp; j--) //드모르간 법칙 a[j] = a[j-1]; a[j] = tmp; } } public static void main(String[] args) { Scanner stdIn =...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/javachap06-InsertionSort/",
        "teaser": null
      },{
        ...
        "title": "[JAVA]chap06-셸정렬 & chap07-집합",
        "excerpt":"```java package chap06; import java.util.*; public class ShellSort { static void shellSort(int[] a, int n) { for(int h = n / 2; h > 0; h /= 2) for(int i = h; i = 0 && a[j] > tmp; j -= h) a[j + h] = a[j]; a[j + h]...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/javachap06-sellsort-&-chap07/",
        "teaser": null
      },{
        ...
        "title": "[c] 기초",
        "excerpt":"# C언어는 메모리와 하드웨어를 직접 제어할 수 있음 > 실행 순서 : 소스코드 편집 > 컴파일 >실행 *파일의 변화* 윈도우 기준 : hello.c -> hello.obj -> hello.exe # 기초 ```c #include //include <= 헤더파일을 포함하는 문법, stdio.h <= printf 함수를 사용할 때 필요한 헤더파일 int main() //main() <= C 언어...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/c-start/",
        "teaser": null
      },{
        "title": "[c] 기초2-문자,상수,덧셈 등등",
        "excerpt":"# 상수와 리터럴 ```c int main8() { const int con1 = 10; // con1 => 상수, 10 => 리터럴:고유의 값 //리터럴 사용하기 printf(\"%d\\n\", 10); //10이 리터럴 값 printf(\"%f\\n\", 01.f); printf(\"%c\\n\", 'a'); //문자는 작은따옴표 printf(\"%s\\n\", \"Hello, world!\"); //문자열은 큰 따옴표를 사용해야함 //상수사용하기 const int con2 = 1; //상수는 항상 선언과 동시에...","categories": [],
        "tags": [],
        ...
        "excerpt":"# 증감 연산자 ```c int main11() { int num1 = 1; int num2 = 2; num1++; // num1 = num1 + 1; num2--; // num2 = num2 - 1; printf(\"%d\\n\", num2); printf(\"%d\\n\", num1); //실수형 float num3 = 2.1f; float num4 = 2.1f; num3++; num4--; printf(\"%f %f\\n\", num3, num4); //3.10000000, 1.10000000...","categories": [],
        "tags": [],
        "url": "http://localhost:4000/c3-if/",
        "teaser": null
      }]
